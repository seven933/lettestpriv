...
class CheckoutController extends GetxController implements GetxService {
  ...
  void setAmountCashOnDelivery(double cashOnDeliveryAmount){

    _cashOnDeliveryAmount = cashOnDeliveryAmount;
    update();
  ...
  Future<String> placeOrder(PlaceOrderBodyModel placeOrderBody, int? zoneID, double amount, double? maximumCodOrderAmount, bool fromCart, bool isCashOnDeliveryActive, {bool isOfflinePay = false}) async {
    ...
    String orderID = '';
    Response response = await checkoutServiceInterface.placeOrder(placeOrderBody, _orderAttachment);
  ...
  void callback(
    ...
    if(isSuccess) {
      ...
      // abre tela do pagamento digital com pix 
      if(paymentMethodIndex == 5){

        //Get.toNamed(RouteHelper.getPixPaymentScreen());

      }

      // Abre o checkout padrão do pagamento digital
