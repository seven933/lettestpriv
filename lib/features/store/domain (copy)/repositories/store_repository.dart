...
import 'package:sixam_mart/features/store/domain/models/card_brand_model.dart';
...
class StoreRepository implements StoreRepositoryInterface {
  ...
  /**
  * Pega lista de bandeiras de cartões aceitas pela loja atrelada ao ID passado por parâmetro.
  */
  @override
  Future<List<CardBrandModel>?> getCardBrandsList(int? storeId) async{

    List<CardBrandModel>? cardBrandList;

    Response response = await apiClient.getData('${AppConstants.cardBrandsList}$storeId');

    if(response == 200){

      cardBrandList = [];
      response.body['card_brands'].forEach((cardBrand) => cardBrandList!.add(CardBrandModel.fromJson(cardBrand)));
